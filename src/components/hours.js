    ...
    <div className={styles.container}>
        ...
        <div className={styles.hoursList}>
            {hoursToDisplay.map((hour, index) => {
                if (!hour || !hour.main || !hour.weather || !Array.isArray(hour.weather) || hour.weather.length === 0) {
                        return null; // Bỏ qua mục không hợp lệ
                }
                const date = new Date(hour.dt * 1000);
                const time = date.toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' });
                const temp = isCelsius ? Math.round(hour.main.temp || 0) : Math.round(((hour.main.temp || 0) * 9/5) + 32);
